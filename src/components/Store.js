...
const Store = ({ tokens, ownedSkins, handlePurchase, currentSkin, setCurrentSkin, highScore = 0, setOwnedSkins }) => {
  ...
  useEffect(() => {
    ...
    if (earnedDailySkin && !ownedSkins.includes('challenge_champion') && setOwnedSkins) {
  ...
  const handleClaimSkin = (skin) => {
    ...
    setOwnedSkins((prevOwned) => {
      if (prevOwned.includes(skin.id)) {
        return prevOwned;
      }
      const updatedOwned = [...prevOwned, skin.id];
      return updatedOwned;
    });
  ...
  const getRequirementStatus = (skin) => {
    ...
    if (skin.unlockType === 'highScore') {
      ...
      const unlocked = highScore >= requiredScore;
      const progressText = `Best score: ${Math.min(highScore, requiredScore)}/${requiredScore}`;
      return {
        unlocked,
